...
;;   (defentity* '..name.. '[(:defaults [:field (fn [] ..result..)])]) => (contains {:defaults {"field" '(fn [] ..result..)}})

;;   (defentity* '..name.. '[(:validators [:field ..regex..])]) =>  (contains {:validators {"field" ..regex..}})
;;   (defentity* '..name.. '[(:validators [:field (fn [] ..result..)])]) => (contains {:validators {"field" '(fn [] ..result..)}})
...
;;                                       :..field2.. (s/both s/Str (s/pred '(fn [] "function")))}
...
;;     (malformed? {:field1 :hello-world :field2 "hello" :field7 [1]}) => falsey
;;     (malformed? {:field2 "hello"} ) => truthy)
;; (fact "checks value type"
;;   (malformed? {:field1 "not-keyword" :field2 "hello" :field7 [1 2]}) => truthy
...
;;       (create (get-conn) {:field1 :alongkeyword :field2 "hello"}) => (contains {:field2 "hello"}))
;;     (fact "input field with nil value should be set"
;;       (create (get-conn) {:field1 :alongkeyword :field2 nil}) => (contains {:field2 nil}))
;;     (fact "input field with false boolean value should be set to false " ; field3 defaults to true
;;       (create (get-conn) {:field1 :alongkeyword :field3 false}) => (contains {:field3 anything}))))
