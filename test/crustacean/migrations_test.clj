(ns crustacean.migrations-test
  (:require [crustacean.migrations :refer :all]
            [datomic.api :as d]
            [crustacean.core :refer [defmodel*]]
...
(deftest test-migration-txes
  (let [model (defmodel* 'model
    ...
    ;; If we migrate a model with no changes, but ask to regenerate db funcs, we get the new db funcs
    (let [txes (migration-txes (last-migration model) model true)
      ...
      (is (contains? tx-idents :model/upsert))
    ...
    (let [model (defmodel* 'model
                  '((:fields [field1 :keyword :unique-value :assignment-required])))
          model-with-defaults (defmodel* 'model
                                '((:fields [field1 :keyword :unique-value :assignment-required])
    ...
    (let [model (defmodel* 'model
                  '((:fields [field1 :keyword :unique-value :assignment-required])))
          model-with-validators (defmodel* 'model
                                  '((:fields [field1 :keyword :unique-value :assignment-required])
    ...
    (let [model (defmodel* 'model
                  '((:fields [field1 :keyword :unique-value :assignment-required])))
          model-with-db-func (defmodel* 'model
                               '((:fields [field1 :keyword :unique-value :assignment-required])
